...
exports.createOrder = async (req, res) => {
    try {
        ...
        const orderData = await createOrderService(fastfoodId, {
            clientName,
            items,
            total,
        ...
        io.to(fastfoodId).emit("newOrder", { message: 'Nouvelle commande ajoutée', orderData });
        res.status(201).json({
        ...
        res.status(500).json({ message: 'Erreur serveur lors de l\'ajout de la commande.' });
    }
};
