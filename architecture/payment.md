...
5. Avant l'appel `/pay`, le service génère un **`paymentRef` unique** (`generateId()`) qui sert
   de clé primaire interne pour la ligne `pending_payments`. Ce ref est **distinct du
   `mw_transaction_id`** (attribué par MobileWallet en retour) — il garantit l'unicité de chaque
   tentative de paiement côté backend, même si MobileWallet renvoie un tx_id différent ou nul.
   Après réponse MobileWallet, on stocke `mw_transaction_id` dans la ligne via
   `repos.pendingPayments.setMwTransactionId(paymentRef, mw_transaction_id)`.
6. Le service **persiste le contexte en BD Supabase** via
   `repos.pendingPayments.create(paymentRef, { userId, items, amount, ... })`
...
7. Renvoie `{ success:true, status:'ussd_sent', mw_transaction_id, payment_number }`.
...
- **`repos.menus`** (`supabase/menus.repo.js`) — `getRawStock(id)` : stock brut (null = illimité)
  pour le pré-check.
- **`repos.pendingPayments`** (`supabase/pendingPayments.repo.js`) — `create(paymentRef, ctx)`,
  `setMwTransactionId(paymentRef, mwTxId)`, `getByMwTransactionId(mwTxId)`,
  `getLatestByUser(userId)`, `markSettled(paymentRef, status)`.
...
- **`mwVerdictService.js`** — `mwVerdictService(payload, source)` :
  ...
  → `createOrderService` (nouvelle commande). Après traitement commandes, crédite chaque marchand
  via `creditMerchantForItem`. Échec partiel toléré.
...
- [x] **`paymentRef` unique** généré côté backend avant `/pay` — clé primaire `pending_payments`,
      indépendante du `mw_transaction_id` MobileWallet (stocké séparément après retour `/pay`).
      Migration `006_pending_payments_payment_ref.sql`.
- [x] **`webhookMobilewallet.service.js` → `mwVerdictService.js`** (renommage) ; les deux canaux
      (webhook + socket) convergent vers ce service unique.
- [x] **Crédit marchand** intégré au verdict : `creditMerchantForItem` crédite chaque marchand
      après confirmation commande (via `creditMerchant.service.js`).
