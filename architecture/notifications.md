...
### Scenario : utilisateur avec tokens Expo ET FCM natifs

**Setup utilisateur** :
```
users/{uid}.fcmTokens = [
  "ExponentPushToken[abc123...]",    # Expo Go ou Expo dev client
  "d_aBcxyz123...",                  # FCM natif (prod build / dev build)
  "ExponentPushToken[def456...]",    # Autre appareil Expo
]
```

**Flux POST /notification/add** (avec logs) :
```
1️⃣  POST /notification/add { userId, title: "Test", body: "Message", type: "order_status" }
    └─ postNotificationController
       ├─ LOG: "📱 Récupération tokens pour userId=${userId}"
       └─ getUserTokens(userId) → Firestore fcmTokens[]

2️⃣  postNotificationService
    ├─ LOG: "📊 Tokens chargés : ${tokens.length} token(s)"
    ├─ tokens = [
    │   "ExponentPushToken[abc123...]",
    │   "d_aBcxyz123...",
    │   "ExponentPushToken[def456...]",
    │ ]
    ├─ validateNotificationData → ✅ OK
    └─ Crée notif Firestore

3️⃣  sendPushToAll(...) — Promise.allSettled
    │
    ├─ Token 1: "ExponentPushToken[abc123...]"
    │  └─ LOG: "🟠 EXPO PUSH → ${token.substring(0,30)}..."
    │     └─ sendExpoPushNotification
    │        └─ POST https://exp.host/--/api/v2/push/send
    │           └─ LOG: "✅ EXPO OK (status 200, response_id=${responseId})"
    │              ou LOG: "❌ EXPO FAIL (DeviceNotRegistered)"
    │
    ├─ Token 2: "d_aBcxyz123..."
    │  └─ LOG: "🔵 FCM NATIVE → ${token.substring(0,30)}..."
    │     └─ admin.messaging().send()
    │        └─ LOG: "✅ FCM OK (response=${response})"
    │           ou LOG: "❌ FCM FAIL (messaging/registration-token-not-registered)"
    │
    └─ Token 3: "ExponentPushToken[def456...]"
       └─ LOG: "🟠 EXPO PUSH → ${token.substring(0,30)}..."
          └─ sendExpoPushNotification
             └─ LOG: "✅ EXPO OK (status 200, response_id=${responseId})"

4️⃣  Cleanup (sur erreurs détectées)
    ├─ LOG: "🧹 Nettoyage tokens stales : [${staleTokens.join(', ')}]"
    └─ cleanStaleTokens(userId, stales)
       └─ arrayRemove sur users/{uid}.fcmTokens
          └─ LOG: "✅ Tokens stales retirés de Firestore"

5️⃣  Socket emit
    ├─ LOG: "📡 Émission socket newNotification → userId=${userId}"
    └─ io.to(userId).emit('newNotification', { notification })

6️⃣  Response
    └─ 200 { success: true, data: {...}, message: "Notification ajoutée avec succès" }
```

### Logs recommandés

Ajouter ces logs à **`sendPushNotification.service.js`** :

```js
const sendPushNotification = async ({ token, title, body, data = {} }) => {
  if (typeof token === 'string' && token.startsWith('ExponentPushToken[')) {
    const shortToken = token.substring(0, 40) + '...';
    console.log(`🟠 EXPO PUSH → ${shortToken}`);
    return sendExpoPushNotification({ token, title, body, data });
  }

  const shortToken = token.substring(0, 40) + '...';
  console.log(`🔵 FCM NATIVE → ${shortToken}`);
  
  try {
    const response = await admin.messaging().send(message);
    console.log(`✅ FCM OK → token=${shortToken}, response=${response}`);
    return { success: true, response };
  } catch (error) {
    console.log(`❌ FCM FAIL → token=${shortToken}, error=${error.message}`);
    return { success: false, error: error.message };
  }
};
```

À **`postNotification.service.js`** :
...
const sendPushToAll = async (title, body, pushData) => {
  ...
  console.log(`📊 Envoi push à ${targetTokens.length} token(s) : ${targetTokens.map(t => t.substring(0,30)).join(', ')}`);
  ...
  const staleTokens = [];
  results.forEach((r, i) => {
    if (isStaleTokenError(r.status === 'fulfilled' ? r.value : null)) {
      staleTokens.push(targetTokens[i]);
    }
  });
  
  if (staleTokens.length > 0) {
    console.log(`🧹 ${staleTokens.length} token(s) stale détecté(s), nettoyage...`);
    await cleanStaleTokens(userId, staleTokens);
    console.log('✅ Cleanup Firestore terminé');
  }
};
```
